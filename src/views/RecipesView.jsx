...
import { defaultGroups, calcRequisition } from '../data/mockData';
...
export default function RecipesView({
    recipes,           // all recipes (mock + custom) from App state
    selectedRecipe,
    setSelectedRecipe,
    cart,
    onAddToCart,
    onCreateNew,       // callback → go to create view
    onEditRecipe,      // callback(recipe) → go to edit view
    onDeleteRecipe,    // callback(recipeId)
}) {
    const [search, setSearch] = useState('');
    const [groups, setGroups] = useState(defaultGroups);
    const [useSubstitutions, setUseSubstitutions] = useState(false);
    const [generated, setGenerated] = useState(false);
    const [deleteConfirm, setDeleteConfirm] = useState(null); // id of recipe pending delete
    ...
    const handleSelectRecipe = (r) => {
        setSelectedRecipe(r);
        setGenerated(false);
        setGroups(defaultGroups); // reset counts on recipe change
    };
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 24 }}>
                ...
                <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        ...
                        {filtered.length === 0 && (
                            <div style={{ textAlign: 'center', padding: 24, color: '#9b6dca', fontSize: 13 }}>
                                No recipes found 🔍
                            </div>
                        )}
                            ...
                            <div key={r.id} style={{ position: 'relative' }}>
                                <button
                                    ...
                                    style={{
                                        ...
                                        transition: 'all 0.2s',
                                        paddingRight: r.isCustom ? 70 : 14, // space for action buttons
                                    }}>
                                    ...
                                    <div style={{ minWidth: 0 }}>
                                        <div style={{ fontWeight: 600, fontSize: 13, color: '#3d1a78', display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
                                            ...
                                            {r.isCustom && (
                                                <span style={{ fontSize: 9, background: '#e8d5f5', color: '#6b3fa0', borderRadius: 10, padding: '1px 6px', fontWeight: 700 }}>
                                                    CUSTOM
                                                </span>
                                            )}
                                        </div>
                                ...
                                {r.isCustom && (
                                    <div style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', display: 'flex', gap: 4 }}>
                                        <button
                                            onClick={e => { e.stopPropagation(); onEditRecipe(r); }}
                                            title="Edit recipe"
                                            style={{
                                                background: 'rgba(107,63,160,0.1)', border: 'none', borderRadius: 8,
                                                width: 28, height: 28, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
                                                color: '#6b3fa0', transition: 'all 0.2s',
                                            }}
                                        >
                                            <Pencil size={13} />
                                        </button>
                                        <button
                                            ...
                                            title={deleteConfirm === r.id ? 'Click again to confirm' : 'Delete recipe'}
                                            style={{
                                                background: deleteConfirm === r.id ? '#fef2f2' : 'rgba(239,68,68,0.1)',
                        ...
                        {/* CTA button at bottom of list */}
                        <button
                            onClick={onCreateNew}
                            style={{
                                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                                padding: '11px 14px', borderRadius: 14, cursor: 'pointer', marginTop: 4,
                                background: 'transparent',
                                border: '1.5px dashed rgba(107,63,160,0.35)',
                                color: '#9b6dca', fontSize: 13, fontWeight: 600,
                                transition: 'all 0.2s',
                            }}
                            onMouseEnter={e => { e.currentTarget.style.borderColor = '#6b3fa0'; e.currentTarget.style.color = '#6b3fa0'; e.currentTarget.style.background = 'rgba(107,63,160,0.05)'; }}
                            onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(107,63,160,0.35)'; e.currentTarget.style.color = '#9b6dca'; e.currentTarget.style.background = 'transparent'; }}
                        >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    {selectedRecipe ? (
                        ...
                        <div className="glass-card" style={{ padding: 40, textAlign: 'center' }}>
                            ...
                            <button onClick={onCreateNew} className="btn-primary" style={{ margin: '0 auto' }}>
                                <Plus size={16} /> Or create your own
                            </button>
                        </div>
                    )}
                                ...
                                {selectedRecipe.ingredients.slice(0, 1).map(ing => {
                                    ...
                                    return (
                                        <div key={ing.id} style={{ fontSize: 11, color: '#4b5563', lineHeight: 1.8 }}>
                                            <b>D</b> = Σ(Pi×Ci) = <b>{r.D.toLocaleString()} {ing.unit}</b><br />
                                            <b>D_safe</b> = D × 1.10 = <b>{r.D_safe.toLocaleString()} {ing.unit}</b><br />
